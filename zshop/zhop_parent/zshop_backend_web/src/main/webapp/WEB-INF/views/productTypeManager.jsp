...
<head>
    ...
    <script src="${APP_PATH}/js/userSetting.js"></script>
    <script src="${APP_PATH}/js/bootstrap-paginator.js"></script>
    <script src="${APP_PATH}/layer/layer.js"></script>
    <link rel="stylesheet" href="${APP_PATH}/css/zshop.css"/>
    <script>
        $(function () {
            $('#pagination').bootstrapPaginator({
                ...
                pageUrl: function (type, page, current) {
                    return '${APP_PATH}/backend/productType/findAll?pageNum=' + page;
                },
        ...
        function addProductType() {
            $.post(
                '${APP_PATH}/backend/productType/add',
                ...
                function (result) {
                    if (result.status == 1) {
                        layer.msg(result.message, {
                            ...
                            location.href = '${APP_PATH}/backend/productType/findAll?pageNum=' +${pageInfo.pages+1};
                        });
                    } else {
                        layer.msg(result.message, {
                            time: 750,
        ...
        function showProductType(id) {
            $.post(
                '${APP_PATH}/backend/productType/findById',
                {'id': id},
                function (result) {
                    if (result.status == 1) {
        ...
        function modifyName() {
            $.ajax({
                type: 'post',
                url: '${APP_PATH}/backend/productType/modifyName',
                data: {
                    'id': $('#proTypeNum').val(),
                ...
                success: function (result) {
                    if (result.status == 1) {
                        layer.msg(result.message, {
                            ...
                            //重新加载数据
                            location.href = '${APP_PATH}/backend/productType/findAll?pageNum=' +${pageInfo.pageNum};
                        });
                    } else {
                        layer.msg(result.message, {
                            time: 750,
        ...
        function deleteProductType() {
            $.get(
                '${APP_PATH}/backend/productType/removeById',
                ...
                function (result) {
                    if (result.status == 1) {
                        layer.msg('删除成功！', {
                            ...
                            //如果当前只剩下一条数据，删除成功后应该跳转到当前页面的前一页
                            if (${pageInfo.size <= 1}) {
                                console.log(${pageInfo.size});
                                location.href = '${APP_PATH}/backend/productType/findAll?pageNum=' +${pageInfo.pageNum-1};
                            } else {
                                //重新加载数据
                                location.href = '${APP_PATH}/backend/productType/findAll?pageNum=' +${pageInfo.pageNum};
                            }
        ...
        function modifyStatus(id, btn) {
            $.get(
                '${APP_PATH}/backend/productType/modifyStatus',
                {'id': id},
                function (result) {
                    if (result.status == 1) {
                        //location.href = '${APP_PATH}/backend/productType/findAll?pageNum=' +${pageInfo.pageNum};
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary updateProType" onclick="deleteProductType()" data-dismiss="modal">删除
                </button>
                <button class="btn btn-primary cancel" data-dismiss="modal">取消</button>
            </div>
        </div>
